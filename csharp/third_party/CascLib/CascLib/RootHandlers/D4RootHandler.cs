...
namespace CASCLib
{
    public class D4RootHandler : TVFSRootHandler
    {
        ...
        // D4Scanner (vendored fork): expose the CoreTOC so callers can look up / enumerate SNOs
        // (e.g. resolve a Maxroll icon snoId -> Texture entry) without reflection.
        public CoreTOCParserD4 TocParser => tocParser;

        // D4Scanner: open a raw TVFS path directly (e.g. "Base\\payload\\<snoId>") without needing the
        // friendly storage tree to be built. Returns null if the path isn't in the local index.
        public Stream OpenByPath(string path)
        {
            var entries = GetVfsRootEntries(Hasher.ComputeHash(path));
            if (entries == null || entries.Count == 0) return null;
            return cascHandler.OpenFile(entries[0].eKey);
        }

        public bool HasPath(string path)
        {
            var e = GetVfsRootEntries(Hasher.ComputeHash(path));
            return e != null && e.Count > 0;
        }

        // D4Scanner: open by a precomputed path hash (e.g. a CASCFile.Hash from the friendly tree).
        public Stream OpenByHash(ulong hash)
        {
            var entries = GetVfsRootEntries(hash);
            if (entries == null || entries.Count == 0) return null;
            return cascHandler.OpenFile(entries[0].eKey);
        }
