...
## v0.105.0 — 2026-06-18
- Capture & updater audit — nine fixes across the TTS-shim installer, the OCR/screen-capture path, Maxroll import, and the auto-updater (all adversarially code-reviewed). Most user-visible: the shim version check now matches the order Diablo IV actually loads the DLL, so a stale copy sitting in the game folder is correctly flagged for upgrade instead of being masked by a newer copy elsewhere; and installing via the PATH fallback no longer rewrites your Windows user PATH in a way that flattened `%USERPROFILE%`-style entries to fixed paths (and downgraded its expandable type).
- The auto-updater no longer stages a truncated or empty download as a valid update (which could leave the app unable to relaunch after "updating"); a short download is now rejected and retried. Importing a Maxroll build during the app's first-run game-data download no longer fails with a file error.
- Smaller capture fixes: a real item icon no longer gets stuck as a silhouette for the session after a transient game-data read race; a held item tooltip is re-read after a momentary OCR hiccup instead of being skipped until it changes on screen; and a couple of native-bitmap / JSON memory leaks on error paths were closed.
